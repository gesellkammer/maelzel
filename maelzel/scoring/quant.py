...
def _evalGridError(profile: QuantizationProfile,
    ...
    for s in snappedEvents:
        ...
        if origdur > 0 and s.duration == 0:
            numGraces += 1
            offsetErr *= graceOffsetFactor
            durErr = abs(origdur - gracedur) / beatdurf
        else:
            if n.isRest:
                offsetErr *= restOffsetErrW / offsetErrW
            durErr = abs(origdur - s.duration) / beatdurf

        totOffsetErr += offsetErr
        totDurErr += durErr

    graceErr = numGraces / len(snappedEvents)
    error = sqrt(totOffsetErr*totOffsetErr * offsetErrW +
                 totDurErr*totDurErr * profile.durationErrorWeight +
                 graceErr*graceErr * profile.graceErrorWeight)
...
class QuantizedBeat:
    ...
    def __init__(self,
        ...
        for n in self.notations:
            assert n.hasRegularDuration(), f"Invalid duration: {n}, dur: {n.duration}, ratios: {n.durRatios}, div: {self.divisions}"
    ...
    def _applyDurationRatios(self):
        ...
        quantutils.applyDurationRatio(self.notations, division=self.divisions,
                                      beatOffset=self.offset, beatDuration=self.duration)
...
class QuantizedMeasure:
    ...
    def beatStructure(self) -> list[QuantizedBeatDef]:
        """
        The beat structure of this measure

        Returns:
            a list of beat definitions.
        """
    ...
    def beatWeights(self) -> list[int]:
        """
        The weights of the beats in this measure
        """
    ...
    def _splitWeakBeat(self, n: Notation, node: Node, offset: F) -> bool:
        ...
        if binary:
            ...
            if leftdur == rightdur and partdur.numerator in (3, 7, 15):
                logger.debug("Too complex syncopation 4")
                return True

        if partprop <= qprofile.syncopPartMinFraction:
            logger.debug("Part of %s, %s too short in relation to the beat, split it!, min fraction: %s, ratio: %s", n, partprop, qprofile.syncopPartMinFraction, nodeDurRatio)
            return True
    ...
    def breakBeam(self, beat: F | float | int, minimum=0, maximum=0) -> None:
        """
        Break or subdivide beams

        Args:
            beat: the location at which to break/subdivide beams
            minimum: if given, the minimum limit of beam subdivision. The number
                identifies the beam to subdivide, where 8=eighth note, 16=16th note, etc.
            maximum: maximum limit of beam subdivision. See above

        .. seealso:: :meth:`QuantizedMeasure.setBeamSubdivisions`
        """
    ...
    def setBeamSubdivisions(self,
        ...
        .. seealso:: :meth:`QuantizedPart.breakBeam`
...
def quantizeBeat2(eventsInBeat: list[Notation],
    ...
    simplifier = quantutils.SubdivSimplifier(events0)

    if searchExact:
        perfectFits = set()
        t0 = time.time()
        fitchecker = quantutils.DivisionFitChecker(eventsInBeat, beatDuration=beatDuration, offset=beatOffset)
        for div in possibleDivisions:
            if div in perfectFits:
                continue
            numSlots = sum(div)
            if numSlots < numRealNotes:
                continue
            if fitchecker.fits(div):
            # if quantutils.divisionFitsPerfectly(div, eventsInBeat, beatDuration=beatDuration, offset=beatOffset):
                if len(div) > 1:
                    div2 = simplifier.simplify(div)
        ...
        if perfectFits:
            possibleDivisions = list(perfectFits)
            possibleDivisions.sort(key=lambda div: sum(div))
            perfectFitsRegistry = perfectFits
    ...
    seen = set()
    quantizer = quantutils.BeatQuantizer(events0, beatDuration=beatDuration, offset=F0)
    ...
    for div in possibleDivisions:
        ...
        div, assignedSlots = quantizer.assignAndSimplify(div)
        if div in seen:
            continue
        seen.add(div)
        ...
        if profile.debug:
            debuginfo = (f"{gridError=:.3g} (weight={gridErrorWeight}), {rhythmComplexity=:.3g} (weight={profile.rhythmComplexityWeight}) ({rhythmInfo}), "
                         f"{divPenalty=:.3g} (weight={profile.divisionErrorWeight} ({divPenaltyInfo})")
        else:
            debuginfo = ''
        # ----------------------------- end
        ...
        elif profile.debug and totalError / minError < 2:
            # keep best rows for debugging
            rows.append((totalError, div, snappedEvents, assignedSlots, debuginfo))
    ...
    if profile.debug:
        ...
        print(f"Solutions in order of eval: {[row[1] for row in rows[:20]]}")
...
def _mergeUnquantizedNotations(notations: list[Notation]) -> list[Notation]:
    """
    Consolidate rests which can be merged together, prior to quantization
...
def _mergeNodes(node1: Node,
    ...
    if any(isinstance(item, Node) for item in node.items):
        if merged := _mergeSiblings(node, profile=profile, beatOffsets=beatOffsets, beatWeights=beatWeights):
            node = merged
...
def _nodesCanMerge(g1: Node,
    ...
    sumdur = g1dur + g2dur
    if g1.durRatio != (1, 1) and acrossBeat:
        ...
        #     return Result.Fail("incompatible duration")
            ...
            return Result.Fail(f"3:2 tuplet across beats results in irregular duration: {g1dur+g2dur}")
    ...
    if isinstance(item1, Node):
        ...
        assert isinstance(item2, Notation)
        # Two Notations
        ...
        assert ispowerof2(symdur.denominator)
        if syncopated and (symdur.denominator > 1 and symdur.numerator not in (1, 2, 3, 4, 7, 8, 15, 16, 31)):
...
def mergeSiblings(root: Node,
    ...
    newroot = newroot.mergedNotations()
    return newroot


def _mergeSiblings(node: Node,
                   profile: QuantizationProfile,
                   beatOffsets: Sequence[F],
                   beatWeights: Sequence[int],
                   maxiter=4
    ...
    Args:
        node: the root of a tree of Nodes
        profile: the quantization profile
    ...
    """
    if len(node.items) < 2 or all(isinstance(item, Notation) for item in node.items):
        return None
    ...
    for _ in range(maxiter):
        node2 = _mergeSiblingsInner(node=node, profile=profile,
...
def _mergeSiblingsInner(node: Node,
                        profile: QuantizationProfile,
                        beatOffsets: Sequence[F],
                        beatWeights: Sequence[int]
                        ) -> Node | None:
    nodeitems = node.items
    if len(nodeitems) < 2 or all(isinstance(item, Notation) for item in node.items):
        return None
    ...
    if isinstance(items1, Node):
        if items1b:=_mergeSiblings(items1, profile=profile, beatOffsets=beatOffsets, beatWeights=beatWeights):
            changed = True
            items1 = items1b

    items = [items1]
    for item2 in nodeitems[1:]:
        if isinstance(item2, Node):
            if item2b:=_mergeSiblings(item2, profile=profile, beatOffsets=beatOffsets, beatWeights=beatWeights):
                item2 = item2b
                changed = True
        item1 = items[-1]
    ...
    if sum(1 for it in items if isinstance(it, Node)) >= 3:
        items3, changed3 = _mergeSiblingsThree(items, root=node, profile=profile,
                                              beatOffsets=beatOffsets, beatWeights=beatWeights)
        if changed3:
            items = items3
            changed = True
    ...
    node2 = Node(ratio=node.durRatio, items=items, parent=node.parent)
    assert node.totalDuration() == node2.totalDuration()
    node2.setParentRecursively()
    return node2


def _mergeSiblingsThree(items: list[Node|Notation],
                        root: Node,
                        profile: QuantizationProfile,
                        beatOffsets: Sequence[F],
                        beatWeights: Sequence[int]) -> tuple[list[Node|Notation], bool]:
    """
...
class QuantizedPart:
    ...
    def measure(self, idx: int, extend=True) -> QuantizedMeasure | None:
        """
        ...
        Returns:
            ...
            empty QuantizedMeasure will be created (and all the measures in
            between)
        """
        ...
        if idx > numMeasures - 1:
            if not extend:
                return None
            self.addEmptyMeasures(numMeasures - idx)
    ...
    def addEmptyMeasures(self, num: int) -> None:
        """Add the given number of empty measures at the end"""
        if num <= 0:
            return
        N = len(self.measures)
        for measureIndex in range(N - 1, N - 1 + num):
        ...
        for measure in self.measures:
            for n in measure.notations():
                if n.isRest or len(n.pitches) <= 1:
                    continue
                notenames = n.resolveNotenames(fixedAnnotation=True)
    ...
    def addSpanner(self, spanner: _spanner.Spanner | str, start: beat_t, end: beat_t
        ...
        """
        n0 = self.notationAt(start)
        n1 = self.notationAt(end)
        n0.addSpanner(spanner, n1)

    def notationAt(self, beat: beat_t) -> Notation:
        """
        ...
        Returns:
            the notation at the given location. Within a quantized part
            a notation is always included within the boundaries of the measure it
            belongs to. The notation itself might be tied to a previous/next notation.

        Raises: ValueError if the given beat is outside this part
        """
    ...
    def breakBeam(self, location: beat_t, minimum: int | None = None, maximum: int | None = None
                  ) -> None:
        """
        ...
        Args:
            location: the beat or location as tuple (measureindex, relative beat) to
                break beams at
            minimum: if given, the min. beam to break. A beam is indicated by its
                denominator (8=eighth note, 16=16th note, etc
            maximum: if given, the max. beam to break. See above

        """
        measure, relbeat = self.measureAt(location)
        if maximum is minimum is None:

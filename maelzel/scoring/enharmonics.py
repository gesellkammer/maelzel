...
def _chordPenalty(notations: Sequence[Notation],
    ...
    for i, notation in enumerate(notations):
        notes = notation.resolveNotenames(fixedAnnotation=False)
        anchor = anchors[i]
...
class SpellingHistory:
    ...
    def addNotation(self, notation: Notation):
        ...
        """
        notenames = notation.resolveNotenames(fixedAnnotation=False)
        self.add(notenames)
    ...
    for group in iterlib.window_fixed_size(notations, groupSize, options.groupStep):
        ...
        for idx, n in enumerate(group):
            if len(n) == 1:
                ...
                chordVariants = pt.enharmonic_variations(n.resolveNotenames(fixedAnnotation=False),
    ...
    for n0, n1 in itertools.pairwise(notations):
        ...
        if n1.tiedPrev:
            if not n0.tiedNext:
                ...
                notenames1 = n1.resolveNotenames(fixedAnnotation=False)
                notenames0 = n0.resolveNotenames(fixedAnnotation=False)
                fixedNotenames = []

...
def _mergeAcrossBeats(groups: list[DurationGroup],
    ...
    """
    After quantization of a measure, no tree extends over the barrier of a beat.
    Here we merge notations across those boundaries for compatible
    totalDuration tree.

    Args:
        groups: a list of totalDuration tree, as returned by QuantizedMeasure.nodes
    ...
    """
    # mark the beginning of each tree. This is needed later on when rebuilding the
...
def _mergeNotationsAcrossBeats(notations: list[Notation],
    ...
    Args:
        ...
        minBeatFractionAcrossBeats: min. fraction of the beat which can result in a merge. If 1, then a merge
            is only performed if the resulting merged Notation has a totalDuration of at least 1 beat
    ...
    for n1 in notations[1:]:
        ...
        mergedDur = n0.totalDuration + n1.totalDuration
        if n0.durRatios == n1.durRatios and core.notationsCanMerge(n0, n1):
            if ((mergedDur < minMergedDuration) or
                (n0.gliss and n0.totalDuration + n1.totalDuration >= 2 and n0.tiedPrev) or
...
def _buildGroups(notations: list[Notation], timesig: timesig_t, mergedTupletsMaxDur: number_t
    ...
    def addToCurrentGroup(n: Notation, stack: GroupStack):
        # TODO: check if the tree is too long
        group = stack.groups[-1]
        if group.durRatio != (1, 1) and group.totalDuration() + n.totalDuration > mergedTupletsMaxDur:
            print("tree too long, splitting", group, n)
            stack.back(1)
    ...
    for n in notations:
        ...
        if backuplevels == 0 and forwardlevels == 0 and n.getProperty('.startGroup'):
            stack.back(1)
            group = stack.groups[-1]
            if group.durRatio != (1, 1) and len(n.durRatios) >= 2 and group.totalDuration() + n.totalDuration > 1:
                stack.back(1)

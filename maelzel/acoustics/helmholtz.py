...
def resonantFrequency(V: float, A= pi*0.01**2, L=0.05, c:float=C) -> float:
    """
    Resonant frequency of a helmholtz-resonator (in Hz)
...
def resonatorVolume(f: float, A=pi * 0.01 ** 2, L=0.05, c=C) -> float:
    """
...
def ductResonantFrequency(L: float, radius: float, n: int, c=C) -> float:
    """
...
def ductLength(freq: float, radius: float, n=1, c=C) -> float:
    """

...
def ringmod(*pitches: pitch_t) -> list[mc.Note]:
    """
    Calculate the ring-modulation between the given notes

    pitches:
        a midinote, a notename or a mc.Note (no frequencies!)
    ...
    all_sidebands.sort()
    return [mc.Note(f2m(sideband)) for sideband in all_sidebands]

def sumtones(*pitches: pitch_t) -> list[mc.Note]:
    """
    ...
    return [mc.Note(f2m(f1+f2)) for f1, f2 in combinations(freqs, 2)]
...
class Difftone:  
    ...
    def notes(self):
        if self._notes is None:
            self._notes = mc.Chord(self.note0, self.note1)
        return self._notes
...
def difftones(*pitches: pitch_t) -> list[mc.Note]:
    """
    ...
    return [mc.Note(f2m(abs(f2 - f1))) for f1, f2 in combinations(freqs, 2)]
    

def difftones_cubic(*notes: pitch_t) -> list[mc.Note]:
    """
...
def ringmod_exactsource(sideband1: pitch_t, sideband2: pitch_t
    ...
    f0 = sumFreq - f1
    return mc.Note(f2m(f0)), mc.Note(f2m(f1))
    ...
    for m0, m1, m2 in combinations(range(int(midi0), int(ceil(midi1))), 3):
        ...
        if len(matching) > len(bestmatch):
            ...
            if newdiff < lastdiff:
                sourcefreqs = [m2f(m) for m in newmidis]
                bestmatch = matching
    return [mc.Note(f2m(freq)) for freq in sourcefreqs]
...
def difftones_beatings(pitch1: pitch_t, pitch2: pitch_t, maxbeatings=20, minbeatings=0.3, wave="saw",
    ...
    pairs2 = [DifftoneBeating(abs(o1-o2), mc.Note(f2m(o1)), mc.Note(f2m(o2)), freq1, freq2, amp)
...
def fm_chord(carrierfreq: float, modfreq: float, index: float, minamp=0.01,
             minpitch=None, maxpitch=None) -> mc.Chord:
...
def fm_chord_ratio(carrier: pitch_t, ratio: float, index: float, minamp=0.01,
                   minpitch=None, maxpitch=None) -> mc.Chord:
    carrierfreq = _asnote(carrier).freq
